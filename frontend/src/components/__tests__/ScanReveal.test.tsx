...
describe('ScanReveal Component', () => {
  ...
  it('calls gsap with correct configuration', () => {
    ...
    // Check that gsap.fromTo was called
    // First call is usually the element reveal
    expect(gsap.fromTo).toHaveBeenCalled();
    
    // Find the call that corresponds to the reveal animation
    // It should have the specific delay passed in props
    const calls = vi.mocked(gsap.fromTo).mock.calls;
    const revealCall = calls.find(call => call[2].delay === 0.5);
  ...
  it('handles reduced motion correctly', () => {
    ...
    // Get the mock instance of matchMedia
    const matchMediaMock = vi.mocked(gsap.matchMedia);
    // The mock returns an object with add, revert.
    // We need to access the return value of the LAST call to matchMedia()
